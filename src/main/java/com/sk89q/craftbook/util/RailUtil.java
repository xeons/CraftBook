...
public final class RailUtil {
    ...
    public static List<Chest> getNearbyChests(Block body) {
        ...
        if (body.getWorld().getBlockAt(x, y, z - 1).getType() == Material.CHEST
                || body.getWorld().getBlockAt(x, y, z - 1).getType() == Material.TRAPPED_CHEST) {
            containers.add((Chest) body.getWorld().getBlockAt(x, y, z - 1).getState());
            if (body.getWorld().getBlockAt(x, y, z - 2).getType() == Material.CHEST
                    || body.getWorld().getBlockAt(x, y, z - 2).getType() == Material.TRAPPED_CHEST) {
    ...
    public static boolean isTrack(Material id) {
        ...
        if (MoreRails.instance != null && MoreRails.instance.pressurePlate) {
            if (id == Material.STONE_PRESSURE_PLATE || Tag.WOODEN_PRESSURE_PLATES.isTagged(id)
                    || id == Material.HEAVY_WEIGHTED_PRESSURE_PLATE || id == Material.LIGHT_WEIGHTED_PRESSURE_PLATE)
                return true;
        }
