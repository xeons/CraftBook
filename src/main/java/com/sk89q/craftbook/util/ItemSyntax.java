...
public final class ItemSyntax {
    ...
    /**
     * The plugin that stores this ItemSyntax reference. Only set this if you have a method: "public String
     * parseItemSyntax(String item) {" in your plugin class.
     */
    public static JavaPlugin plugin;

    /**
     * The opposite of {@link ItemSyntax#getItem(String)}. Returns the String made by an {@link ItemStack}. This can be
     * used in getItem() to return the same {@link ItemStack}.
     *
     * @author me4502
     *
        ...
        if (item.hasItemMeta()) {
            ...
            if (meta instanceof SkullMeta) {
                ...
                if (!((LeatherArmorMeta) meta).getColor().equals(Bukkit.getItemFactory().getDefaultLeatherColor()))
                    builder.append("/color:").append(((LeatherArmorMeta) meta).getColor().getRed()).append(',')
                            .append(((LeatherArmorMeta) meta).getColor().getGreen()).append(',')
                            .append(((LeatherArmorMeta) meta).getColor().getBlue());
            } else if (meta instanceof PotionMeta) {
                if (!((PotionMeta) meta).hasCustomEffects())
                    for (PotionEffect eff : ((PotionMeta) meta).getCustomEffects())
                        builder.append("/potion:").append(eff.getType().getName()).append(';').append(eff.getDuration())
                                .append(';').append(eff.getAmplifier());
            } else if (meta instanceof EnchantmentStorageMeta) {
                if (!((EnchantmentStorageMeta) meta).hasStoredEnchants())
                    for (Entry<Enchantment, Integer> eff : ((EnchantmentStorageMeta) meta).getStoredEnchants()
                            .entrySet())
                        builder.append("/enchant:").append(eff.getKey().getKey().toString()).append(';')
                                .append(eff.getValue());
    ...
    private static final LoadingCache<String, ItemStack> itemCache = CacheBuilder.newBuilder().maximumSize(1024)
            .expireAfterAccess(10, TimeUnit.MINUTES).build(new CacheLoader<String, ItemStack>() {
                ...
                public ItemStack load(String line) throws Exception {
                    ...
                    try {
                        ...
                        if (material != null) {
                            ...
                            try {
                                int type = LegacyMapper.getInstance()
                                        .getLegacyFromItem(BukkitAdapter.asItemType(material))[0];
                                item = new BaseItem(LegacyMapper.getInstance().getItemFromLegacy(type, data));
                            } catch (Exception ee) {
                                CraftBookPlugin.logger()
                                        .warning("Failed to convert legacy item: " + material.getId() + ':' + data);
                                ee.printStackTrace();
                            }
                        }
                    }
                    ...
                    if (advMetadataSplit.length > 1) {
                        ...
                        for (int i = 1; i < advMetadataSplit.length; i++) {
                            ...
                            if (bits[0].equalsIgnoreCase("player") && meta instanceof SkullMeta)
                                ...
                                ((LeatherArmorMeta) meta).setColor(org.bukkit.Color.fromRGB(Integer.parseInt(cols[0]),
                                ...
                                try {
                                    PotionEffect effect = new PotionEffect(PotionEffectType.getByName(effects[0]),
                                            Integer.parseInt(effects[1]), Integer.parseInt(effects[2]));
                                ...
                                try {
                                    ...
                                    ((EnchantmentStorageMeta) meta).addStoredEnchant(ench, Integer.parseInt(sp[1]),
                                            true);
                                } catch (Exception ignored) {
                                }
