...
public final class ParsingUtil {
    ...
    public static String parsePlayerTags(String line, Player player) {
        ...
        line = StringUtils.replace(line, "@p.l",
        ...
        line = StringUtils.replace(line, "@p.i",
                CraftBookPlugin.inst().getUUIDMappings().getCBID(player.getUniqueId()));
    ...
    private static final LoadingCache<String, List<String>> variableFinderCache = CacheBuilder.newBuilder()
            .maximumSize(1024).expireAfterAccess(10, TimeUnit.MINUTES).build(new CacheLoader<String, List<String>>() {
                @Override
                public List<String> load(String line) throws Exception {
                    ...
                    for (String bit : RegexUtil.PERCENT_PATTERN.split(line)) {
                        if (line.indexOf(bit) > 0 && line.charAt(line.indexOf(bit) - 1) == '\\')
                            continue;
    ...
    public static String parseVariables(String line, CommandSender player) {
        ...
        if (CraftBookPlugin.inst() == null || VariableManager.instance == null
                || VariableManager.instance.getVariableStore().isEmpty())
            return line;
        ...
        for (String var : getPossibleVariables(line)) {
            ...
            if (var.contains("|")) {
                ...
                CraftBookPlugin.logDebugMessage("Variable " + value + " at " + key + " detected!",
                        "variables.line-parsing");
            } else {
                key = "global";
                value = var;
