    ...
    private void checkForLanguages() {
        ...
        for (String language : CraftBookPlugin.inst().getConfiguration().languages) {
            ...
            try {
                ...
                CraftBookPlugin.inst().getLogger().severe("An error occured loading the languages file for: " + language
                        + "! This language WILL NOT WORK UNTIL FIXED!");
                e.printStackTrace();
                continue;
            }
        ...
        if (languageData == null) {
            ...
            if (!CraftBookPlugin.inst().getConfiguration().languageScanText || translated != null) {
                ...
                for (String tran : languageData.getMap().keySet()) {
                    String trand = defaultMessages.get(tran) != null
                            ? languageData.getString(tran, defaultMessages.get(tran))
                            : languageData.getString(tran);
                    if (tran == null || trand == null)
                        continue;
