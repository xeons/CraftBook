...
public final class LegacyCustomDropManager {
    ...
    public LegacyCustomDropManager(File source) {
        ...
        if (mobDefinitions.exists()) {
            try {
                ...
                CraftBookPlugin.logger().log(Level.SEVERE, "Unknown IO error while loading custom mob drop definitions",
                        e);
            } catch (Exception e) {
                CraftBookPlugin.logger().log(Level.SEVERE,
    ...
    public CustomItemDrop getBlockDrops(int block) {
        ...
        if (block < 0 || block >= BLOCK_ID_COUNT)
            return null;
        else
            return blockDropDefinitions[block];
    }

    public DropDefinition[] getMobDrop(LivingEntity mob) {

        if (mob.getCustomName() != null && mobDropDefinitions.containsKey(ChatColor.translateAlternateColorCodes('&',
                (mob.getType().name() + "|" + mob.getCustomName()).toLowerCase(Locale.ENGLISH))))
            return mobDropDefinitions.get(ChatColor.translateAlternateColorCodes('&',
                    (mob.getType().name() + "|" + mob.getCustomName()).toLowerCase(Locale.ENGLISH)));
    ...
    public void loadDropDefinitions(File file, boolean isMobDrop) throws IOException {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    if (line.contains("#"))
                        line = RegexUtil.COMMENT_PATTERN.split(line)[0]; // Remove comments
                ...
                if (!isMobDrop) {
                    ...
                    BlockCustomDropDefinition converted = new BlockCustomDropDefinition(
                            sourceId + "" + data + "" + this.converted++, stacks, null, TernaryState.NONE, state);
            ...
            if (isMobDrop) {
                ...
                for (Entry<String, DropDefinition[]> def : mobDropDefinitions.entrySet()) {
                    ...
                    EntityCustomDropDefinition converted = new EntityCustomDropDefinition(
                            def.getKey().toUpperCase() + "" + this.converted++, stacks, null, TernaryState.NONE,
                            EntityType.valueOf(def.getKey().toUpperCase()));
                    converted.setAppend(append);
    ...
    public static class CustomItemDrop {
        ...
        public DropDefinition[] getDrop(int data) {
            ...
            if (data < 0 || data >= DATA_VALUE_COUNT)
                return defaultDrop;
    ...
    public static class DropDefinition {
        ...
        public ItemStack getItemStack() {
            ...
            if (CraftBookPlugin.inst().getRandom().nextInt(100) > chance)
                return null;
            ItemStack stack = this.stack.clone();
            stack.setAmount(countMin == countMax
                    ? countMin
                    : countMin + CraftBookPlugin.inst().getRandom().nextInt(countMax - countMin + 1));
            return stack;
        }
    }
}
