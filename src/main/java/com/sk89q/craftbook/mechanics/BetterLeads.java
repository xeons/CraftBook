...
public class BetterLeads extends AbstractCraftBookMechanic {
    ...
    public void onPlayerClick(final PlayerInteractEntityEvent event) {
        ...
        CraftBookPlugin.logDebugMessage("A player has right clicked an entity with a lead!",
                "betterleads.allowed-mobs");
        ...
        if (!player.hasPermission("craftbook.mech.leads")
                && !player.hasPermission("craftbook.mech.leads.mobs." + typeName.toLowerCase())) {
        ...
        if (event.getRightClicked() instanceof Creature && ((Creature) event.getRightClicked()).getTarget() != null
                && ((Creature) event.getRightClicked()).getTarget().equals(event.getPlayer()))
    ...
    public void onEntityTarget(EntityTargetEvent event) {
        ...
        if (!leadsStopTarget && !leadsMobRepellant)
            return;
    ...
    public void onHitchBreakRandomly(final HangingBreakEvent event) {
        ...
        if (amountConnected == 0) {
            // Still needs to be used by further plugins in the event. We wouldn't want bukkit complaining now, would
            // we?
    ...
    public void onHitchBreak(final HangingBreakByEntityEvent event) {
        ...
        if (!leadsHitchPersists && !leadsOwnerBreakOnly)
            return;
        if (!(event.getEntity() instanceof LeashHitch))
            return;
        ...
        for (Entity ent : event.getEntity().getNearbyEntities(10, 10, 10)) {
            ...
            if (ent instanceof Tameable)
                if (!((Tameable) ent).isTamed() || ((Tameable) ent).getOwner().equals(event.getRemover()))
                    isOwner = true;
            if (isOwner || !(ent instanceof Tameable) || !leadsOwnerBreakOnly
                    || event.getRemover().hasPermission("craftbook.mech.leads.owner-break-only.bypass")) {
    ...
    public void onUnleash(PlayerUnleashEntityEvent event) {
        ...
        if (!leadsOwnerBreakOnly)
            return;
        if (!(event.getEntity() instanceof LivingEntity))
            return;
        if (!((LivingEntity) event.getEntity()).isLeashed()
                || !(((LivingEntity) event.getEntity()).getLeashHolder() instanceof LeashHitch))
            return;
    ...
    public void loadConfiguration(YAMLProcessor config, String path) {
        ...
        config.setComment(path + "stop-mob-target",
                "Stop hostile mobs targeting you if you are holding them on a leash.");
        leadsStopTarget = config.getBoolean(path + "stop-mob-target", false);

        config.setComment(path + "owner-unleash-only",
                "Only allow the owner of tameable entities to unleash them from a leash hitch.");
        leadsOwnerBreakOnly = config.getBoolean(path + "owner-unleash-only", false);

        config.setComment(path + "hitch-persists",
                "Stop leash hitches breaking when clicked no entities are attached. This allows for a public horse hitch or similar.");
        leadsHitchPersists = config.getBoolean(path + "hitch-persists", false);

        config.setComment(path + "mob-repel",
                "If you have a mob tethered to you, mobs of that type will not target you.");
