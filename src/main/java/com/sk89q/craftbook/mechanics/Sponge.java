...
public class Sponge extends AbstractCraftBookMechanic {
        ...
        for (int cx = -radius; cx <= radius; cx++) {
            for (int cy = -radius; cy <= radius; cy++) {
                for (int cz = -radius; cz <= radius; cz++) {
                    ...
                    if (circularRadius && !LocationUtil.isWithinSphericalRadius(sponge.getLocation(),
                            event.getToBlock().getLocation(), radius))
                        continue;
                    if (redstone && !sponge.isBlockIndirectlyPowered())
                        continue;
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        ...
        if (!isValidSponge(event.getBlock()))
            return;
        ...
        for (int cx = -radius; cx <= radius; cx++) {
            for (int cy = -radius; cy <= radius; cy++) {
                for (int cz = -radius; cz <= radius; cz++) {
                    ...
                    if (circularRadius
                            && !LocationUtil.isWithinSphericalRadius(water.getLocation(), block.getLocation(), radius))
                        continue;
    ...
    public void addWater(Block block) {
        ...
        for (cy = block.getY() - radius - 1; cy <= block.getY() + radius + 1; cy++) {
            for (cz = block.getZ() - radius - 1; cz <= block.getZ() + radius + 1; cz++) {
                ...
                if (circularRadius && !LocationUtil.isWithinSphericalRadius(water.getLocation(), block.getLocation(),
                        radius + 1.5))
                    continue;
                if (water.getType() == Material.WATER) {
        ...
        // The negative y edge
        /*
         * cy = block.getY() - radius - 1; for (cx = block.getX() - radius - 1; cx <= block.getX() + radius + 1; cx++) {
         * for (int cz = block.getZ() - radius - 1; cz <= block.getZ() + radius + 1; cz++) { Block water =
         * block.getWorld().getBlockAt(cx, cy, cz); if(CraftBookPlugin.inst().getConfiguration().spongeCircleRadius &&
         * !LocationUtil.isWithinSphericalRadius(water.getLocation(), block.getLocation(), radius+1.5)) continue; if
         * (water.getType() == Material.WATER) { if(BlockUtil.isBlockReplacable(water.getRelative(0, 1, 0).getType())) {
         * BlockFromToEvent event = new BlockFromToEvent(water, water.getRelative(0, 1, 0));
         * Bukkit.getPluginManager().callEvent(event); if(!event.isCancelled()) water.getRelative(0, 1,
         * 0).setType(Material.WATER); } } } }
         */
