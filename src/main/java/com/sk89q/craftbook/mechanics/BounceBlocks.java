...
public class BounceBlocks extends AbstractCraftBookMechanic {
    ...
    public void loadConfiguration(YAMLProcessor config, String path) {
        ...
        blocks = BlockSyntax.getBlocks(
                config.getStringList(path + "blocks", Collections.singletonList(BlockTypes.DIAMOND_BLOCK.id())), true);
    ...
    public void onPlayerMove(final PlayerMoveEvent event) {
        ...
        if (Math.abs(event.getTo().getY() - event.getFrom().getY()) > sensitivity
                && event.getFrom().getY() - event.getFrom().getBlockY() < 0.25) { // Sensitivity setting for the
                                                                                  // jumping, may need tweaking

            if (!event.getPlayer().hasPermission("craftbook.mech.bounceblocks.use")) // Do this after the simple
                                                                                     // arithmatic, permission lookup is
                                                                                     // slower.
                return;
                ...
                if (SignUtil.isSign(sign)) {
                    ...
                    if (s.getLine(1).equals("[Jump]")) {
                        ...
                        if (!straight) {
                            ...
                            // Find out the angle they are facing. This is completely to do with horizontals. No
                            // verticals are taken into account.
            ...
            for (Entry<BaseBlock, Vector> entry : autoBouncers.entrySet()) {
                if (entry.getKey().equalsFuzzy(BukkitAdapter.adapt(block.getBlockData()))) {
                    ...
                    CraftBookPlugin.logDebugMessage("Player jumped on a auto block that is a BoucneBlock!",
                            "bounce-blocks");
                    ...
                    // Find out the angle they are facing. This is completely to do with horizontals. No verticals are
                    // taken into account.
                    double angle = Math.atan2(facing.getX(), facing.getZ());
    ...
    public void onSignChange(SignChangeEvent event) {
        ...
        if (!event.getLine(1).equalsIgnoreCase("[jump]"))
            return;
