...
public class RecipeManager {
    ...
    public void load() {
        ...
        try {
            ...
            CraftBookPlugin.logger().severe(
                    "Corrupt Custom Crafting crafting-recipes.yml File! Make sure that the correct syntax has been used, and that there are no tabs!");
            e.printStackTrace();
        }

        config.setHeader(
                "# CraftBook Custom Recipes. CraftBook Version: "
                        + CraftBookPlugin.inst().getDescription().getVersion(),
                "# For more information on setting up custom recipes, see the wiki:",
                "# " + CraftBookPlugin.getWikiDomain() + "/mechanics/custom_crafting/",
                "",
                "");
    ...
    public final class Recipe {
        ...
        public Recipe(String id, RecipeType type, LinkedHashMap<CraftingItemStack, Character> items, List<String> shape,
                CraftingItemStack result, HashMap<String, Object> advancedData) throws InvalidCraftingException {
            this.id = id;
            this.type = type;
            this.items = items;
            this.shape = shape;
        ...
        public Recipe(String id, RecipeType type, List<CraftingItemStack> ingredients, CraftingItemStack result,
                HashMap<String, Object> advancedData) throws InvalidCraftingException {
            this.id = id;
            this.type = type;
            ...
            if (actions != null && !actions.isEmpty()) {
                ...
                for (String s : actions) {
                    if (s.equalsIgnoreCase("commands-console"))
                        addAdvancedData("commands-console", config
                                .getStringList("crafting-recipes." + id + ".craft-actions." + s, new ArrayList<>()));
                    else if (s.equalsIgnoreCase("commands-player"))
                        addAdvancedData("commands-player", config
                                .getStringList("crafting-recipes." + id + ".craft-actions." + s, new ArrayList<>()));
                }
            }
        }
        ...
        public void save() {
            ...
            if (type != RecipeType.SHAPED) {
                ...
                for (Map.Entry<CraftingItemStack, Character> craftingItemStackCharacterEntry : items.entrySet())
                    resz.put(craftingItemStackCharacterEntry.getKey().toString() + ' ',
                            craftingItemStackCharacterEntry.getValue());
                config.setProperty("crafting-recipes." + id + ".ingredients", resz);
            ...
            if (hasAdvancedData("extra-results")) {
                ...
                ArrayList<CraftingItemStack> extraResults = new ArrayList<>(
                        (Collection<? extends CraftingItemStack>) getAdvancedData("extra-results"));
            ...
            if (hasAdvancedData("commands-player") || hasAdvancedData("commands-console")) {
                ...
                if (hasAdvancedData("commands-player"))
                    config.setProperty("crafting-recipes." + id + ".craft-actions.commands-player",
                            getAdvancedData("commands-player"));
                if (hasAdvancedData("commands-console"))
                    config.setProperty("crafting-recipes." + id + ".craft-actions.commands-console",
                            getAdvancedData("commands-console"));
            }
        }
            ...
            try {
                for (String item : config.getKeys(path)) {
                    ItemStack stack = ItemUtil
                            .makeItemValid(ItemSyntax.getItem(RegexUtil.PERCENT_PATTERN.split(item.trim())[0]));

                    if (stack != null) {
                        stack.setAmount(1);
                        CraftingItemStack itemStack = new CraftingItemStack(stack);
                        if (RegexUtil.PERCENT_PATTERN.split(item).length > 1)
                            itemStack.addAdvancedData("chance",
                                    Double.parseDouble(RegexUtil.PERCENT_PATTERN.split(item.trim())[1]));
        ...
        private List<CraftingItemStack> getItems(String path) {
            ...
            try {
                for (Object oitem : config.getKeys(path)) {
                    ...
                    ItemStack stack = ItemUtil
                            .makeItemValid(ItemSyntax.getItem(RegexUtil.PERCENT_PATTERN.split(item)[0]));
                    ...
                    if (stack != null) {
                        ...
                        if (RegexUtil.PERCENT_PATTERN.split(item).length > 1)
                            itemStack.addAdvancedData("chance",
                                    Double.parseDouble(RegexUtil.PERCENT_PATTERN.split(item)[1]));
                        items.add(itemStack);
                    }
                }
            } catch (Exception e) {
        ...
        public void addAdvancedData(String key, Object data) {
            CraftBookPlugin.logDebugMessage(
                    "Adding advanced data of type: " + key + " to an ItemStack! {" + String.valueOf(data) + '}',
                    "advanced-data.init");
    ...
    public enum RecipeType {
        ...
        public static RecipeType getTypeFromName(String name) {
            if (name.equalsIgnoreCase("Shaped2x2") || name.equalsIgnoreCase("Shaped3x3")) {
                CraftBookPlugin.logger().warning(
                        "You are using deprecated recipe type '" + name + "', we recommend you change it to 'shaped'!");
                return SHAPED;
            }
