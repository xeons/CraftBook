...
public class BetterPlants extends AbstractCraftBookMechanic {
    ...
    public void onBlockBreak(final BlockBreakEvent event) {
        ...
        if (fernFarming && event.getBlock().getType() == Material.LARGE_FERN
                ...
                && ((Bisected) event.getBlock().getRelative(0, -1, 0).getBlockData())
                        .getHalf() == Bisected.Half.BOTTOM) {
            Bukkit.getScheduler().runTaskLater(CraftBookPlugin.inst(), () -> {
                event.getBlock().getWorld().dropItemNaturally(BlockUtil.getBlockCentre(event.getBlock()),
            ...
            for (World world : tickedWorlds) {
                ...
                for (Chunk chunk : world.getLoadedChunks()) {
                    ...
                    if (fastTickRandoms)
                        ...
                        block = chunk.getBlock(CraftBookPlugin.inst().getRandom().nextInt(16),
                                CraftBookPlugin.inst().getRandom().nextInt(world.getMaxHeight()),
                                CraftBookPlugin.inst().getRandom().nextInt(16));
    ...
    public void loadConfiguration(YAMLProcessor config, String path) {
        ...
        config.setComment(path + "fern-farming",
                "Allows ferns to be farmed by breaking top half of a large fern. (And small ferns to grow)");
        fernFarming = config.getBoolean(path + "fern-farming", true);

        config.setComment(path + "fast-random-ticks",
                "Use a way of generating less random numbers, by only generating it once for all chunks, instead of one each chunk.");
