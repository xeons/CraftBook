...
public class Perlstone implements PlcLanguage<boolean[], WithLineInfo<String>[]> {
            ...
            for (int i = 0; i < chars.length; i++) {
                try {
                    ...
                    switch (c) {
                        ...
                        case ']' :
                            if (bracketCount == 0)
                                throw new ICVerificationException("Too many closing braces " + "on line " + li[i].line
                                        + " at column " + li[i].col);
                            bracketCount--;
                            break;
                        case '+' :
    ...
    public void loadState(boolean[] state, DataInputStream in) throws IOException {
        ...
        if (in.readInt() != PERLSTONE_STORE_VERSION)
    ...
    private int decodeAddress(char c, int shift) {
        ...
        if (c >= '0' && c <= '9')
            return mod(c - '0' + shift, 32);
        else
            return mod(c - 'a' + 10 + shift, 32);
    }
