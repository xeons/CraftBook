...
public class BetterPistons extends AbstractCraftBookMechanic {
    ...
    public void onBlockRedstoneChange(SourcedBlockRedstoneEvent event) {
        ...
        for (BlockFace face : LocationUtil.getDirectFaces()) {
            ...
            if (face != BlockFace.UP && face != BlockFace.DOWN
                    && !SignUtil.getBackBlock(sign).getLocation().equals(event.getBlock().getLocation()))
                continue;
            type = checkSign(sign);
            if (type != null)
                types.add(new Tuple2<>(type, sign));
            if (type != null && SignUtil.isSign(sign.getRelative(face))
                    && SignUtil.getFacing(sign.getRelative(face)) == SignUtil.getFacing(sign)) {
    ...
    public void crush(Block trigger, Piston piston, ChangedSign signState) {
        ...
        if (Blocks.containsFuzzy(pistonsCrusherBlacklist,
                BukkitAdapter.adapt(trigger.getRelative(piston.getFacing()).getBlockData()))) {
            return;
        }
    ...
    public void bounce(Block trigger, Piston piston, ChangedSign signState) {
        ...
        Vector vel = new Vector(piston.getFacing().getModX(), piston.getFacing().getModY(),
                piston.getFacing().getModZ()).multiply(mult);
        if (trigger.getRelative(piston.getFacing()).getType() == Material.AIR
                || trigger.getRelative(piston.getFacing()).getState() != null
                ...
                || Blocks.containsFuzzy(pistonsBounceBlacklist,
                        BukkitAdapter.adapt(trigger.getRelative(piston.getFacing()).getBlockData()))) {
            for (Entity ent : trigger.getRelative(piston.getFacing()).getChunk().getEntities()) {
                if (EntityUtil.isEntityInBlock(ent, trigger.getRelative(piston.getFacing()))) {
            ...
            FallingBlock fall = trigger.getWorld().spawnFallingBlock(
                    trigger.getRelative(piston.getFacing()).getLocation().add(vel),
                    trigger.getRelative(piston.getFacing()).getBlockData());
    ...
    public void superSticky(final Block trigger, final Piston piston, final ChangedSign signState) {
        ...
        if (trigger.getRelative(piston.getFacing()).getType() == Material.PISTON_HEAD
                || trigger.getRelative(piston.getFacing()).getType() == Material.MOVING_PISTON) {
            ...
            for (int p = 0; p < amount; p++) {
                ...
                Bukkit.getScheduler().runTaskLater(CraftBookPlugin.inst(), () -> {
                    for (int x = 1; x <= fblock + 2; x++) {
                        int i = x;
                        if (x == 1 && !InventoryUtil.doesBlockHaveInventory(trigger.getRelative(piston.getFacing(), i))
                                && fp == 0) {
                            x = i = 2;
                        }
                        if (x >= fblock + 2
                                || trigger.getRelative(piston.getFacing(), i + 1).getType() == Material.AIR && !air
                                || !canPistonPushBlock(trigger.getRelative(piston.getFacing(), i + 1))) {
                        ...
                        for (Entity ent : trigger.getRelative(piston.getFacing(), i).getChunk().getEntities()) {
                            if (EntityUtil.isEntityInBlock(ent, trigger.getRelative(piston.getFacing(), i))) {
                                ent.teleport(ent.getLocation().subtract(piston.getFacing().getModX() * movemod,
                                        piston.getFacing().getModY() * movemod,
                                        piston.getFacing().getModZ() * movemod));
                            }
                        }
                        copyData(trigger.getRelative(piston.getFacing(), i + 1),
                                trigger.getRelative(piston.getFacing(), i));
                    }
                }, 3L * (p + 1));
            }
        }
            ...
            for (int p = 0; p < amount; p++) {
                Bukkit.getScheduler().runTaskLater(CraftBookPlugin.inst(), () -> {
                    for (int x = fblock + 2; x >= 1; x--) {
                        ...
                        if (trigger.equals(offset) || offset.getType() == Material.MOVING_PISTON
                                || offset.getType() == Material.PISTON_HEAD || !canPistonPushBlock(offset))
                            continue;
                        if (next.getType() == Material.AIR) {
                            for (Entity ent : next.getChunk().getEntities()) {
                                if (EntityUtil.isEntityInBlock(ent, offset)) {
                                    ent.teleport(ent.getLocation().add(piston.getFacing().getModX() * movemod,
                                            piston.getFacing().getModY() * movemod,
                                            piston.getFacing().getModZ() * movemod));
                                }
                            }
    ...
    public void loadConfiguration(YAMLProcessor config, String path) {
        ...
        config.setComment(path + "crushers-kill-mobs",
        ...
        pistonsCrusherBlacklist = BlockSyntax
                .getBlocks(config.getStringList(path + "crusher-blacklist", getDefaultBlacklist()), true);
        ...
        config.setComment(path + "movement-blacklist",
                "A list of blocks that the movement related BetterPistons can not interact with.");
        pistonsMovementBlacklist = BlockSyntax
                .getBlocks(config.getStringList(path + "movement-blacklist", getDefaultBlacklist()), true);
        ...
        pistonsBounceBlacklist = BlockSyntax
                .getBlocks(config.getStringList(path + "bounce-blacklist", getDefaultBlacklist()), true);
