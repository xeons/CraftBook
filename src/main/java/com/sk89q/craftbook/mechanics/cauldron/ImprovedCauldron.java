...
public class ImprovedCauldron extends AbstractCraftBookMechanic {
    ...
    public boolean enable() {
        ...
        CraftBookPlugin.inst().createDefaultConfiguration(
                new File(CraftBookPlugin.inst().getDataFolder(), "cauldron-recipes.yml"), "cauldron-recipes.yml");
        recipes = new ImprovedCauldronCookbook(
                new YAMLProcessor(new File(CraftBookPlugin.inst().getDataFolder(), "cauldron-recipes.yml"), true,
                        YAMLFormat.EXTENDED),
    ...
    private boolean isCauldron(Block block) {
        ...
        if (block.getType() == Material.WATER_CAULDRON && (block.getRelative(BlockFace.DOWN).getType() == Material.FIRE
    ...
    public void onRightClick(PlayerInteractEvent event) {
        ...
        if (!isCauldron(event.getClickedBlock()))
            return;
        CraftBookPlayer player = CraftBookPlugin.inst().wrapPlayer(event.getPlayer());
    ...
    public class ItemTracker extends BukkitRunnable {
        ...
        public ItemTracker(Item item) {
            ...
            lastLocation = new Location(item.getLocation().getWorld(), Integer.MAX_VALUE, Integer.MAX_VALUE,
                    Integer.MAX_VALUE);
            this.item = item;
        }
    ...
    public boolean performCauldron(Block block, CraftBookPlayer player) {
        ...
        try {
            ...
            if (!useSpoons || player == null && allowRedstone) {
                ...
                if (player != null)
                    player.print(
                            "You have cooked the " + ChatColor.AQUA + recipe.getName() + ChatColor.YELLOW + " recipe.");
    ...
    public static boolean isItemSpoon(Material id) {
        ...
        return id == Material.WOODEN_SHOVEL || id == Material.STONE_SHOVEL || id == Material.IRON_SHOVEL
                || id == Material.DIAMOND_SHOVEL || id == Material.GOLDEN_SHOVEL;
    }
    ...
    /**
     * When this is called we know that all ingredients match. This means we destroy all items inside the cauldron and
    ...
    private static Collection<Item> getItems(Block block) {
        ...
        for (Entity entity : block.getChunk().getEntities()) {
            if (entity instanceof Item) {
                if (EntityUtil.isEntityInBlock(entity, block)
                        || EntityUtil.isEntityInBlock(entity, block.getRelative(BlockFace.UP))) {
    ...
    public void loadConfiguration(YAMLProcessor config, String path) {
        ...
        config.setComment(path + "item-tracking",
                "Tracks items and forces them to to tracked by the cauldron. Fixes mc bugs by holding item in place.");
        itemTracking = config.getBoolean(path + "item-tracking", false);

        config.setComment(path + "require-sign",
                "Requires a [Cauldron] sign to be on the side of a cauldron. Useful for requiring creation permissions.");
