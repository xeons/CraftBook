...
/**
 * <p>
 * Collects carts when carts pass over an unpowered chest marked as a dispenser; dispenses carts when power flips on if
 * no cart is already above the dispenser. The dispenser does not impart velocity to dispensed carts, so it is typical
 * to simply place the track above the dispenser so as to be at an angle.
 * </p>
 * <p>
 * Carts of all types (including powered and storage carts) will be collected by this mechanism, but only regular
 * minecarts are dispensed. This may be changed in a future version.
 * </p>
 * <p>
 * Dispenser signs which contain "inf" on the third line will collect carts without storing them and create carts
 * without requiring them from inventory.
 * </p>
 * <p>
 * Note that this is not an exact reimplementation of the mechanics from hmod; this is something new and more consistent
 * with other modern cart mechanisms.
 * </p>
 *
 * @author hash
 */
public class CartDispenser extends CartBlockMechanism {
    ...
    private enum CartType {
        Minecart("Minecart", Minecart.class), StorageMinecart("Storage", StorageMinecart.class), PoweredMinecart(
                "Powered", PoweredMinecart.class), TNTMinecart("TNT", ExplosiveMinecart.class), HopperMinecart("Hopper",
    ...
    public void loadConfiguration(YAMLProcessor config, String path) {
        ...
        config.setComment(path + "spawn-infront",
                "Sets whether the minecarts should spawn infront of the mechanic instead of directly above.");
        minecartDispenserLegacy = config.getBoolean(path + "spawn-infront", false);

        config.setComment(path + "check-for-carts",
                "Sets whether or not the mechanic checks for existing carts before spawning a new one.");
