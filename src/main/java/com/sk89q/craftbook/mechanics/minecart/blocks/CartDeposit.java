...
public class CartDeposit extends CartBlockMechanism {
    ...
    public void onVehicleImpact(CartBlockImpactEvent event) {
        ...
        if (collecting) {
            ...
            CraftBookPlugin.logDebugMessage("collecting " + transferItems.size() + " item stacks",
                    "cart-deposit.collect");
            for (ItemStack stack : transferItems)
                CraftBookPlugin.logDebugMessage(
                        "collecting " + stack.getAmount() + " items of type " + stack.getType().toString(),
                        "cart-deposit.collect");
            ...
            CraftBookPlugin.logDebugMessage("collected items. " + transferItems.size() + " stacks left over.",
                    "cart-deposit.collect");

            leftovers
                    .addAll(cartinventory.addItem(transferItems.toArray(new ItemStack[transferItems.size()])).values());
            transferItems.clear();
            transferItems.addAll(leftovers);
            leftovers.clear();

            CraftBookPlugin.logDebugMessage("collection done. " + transferItems.size() + " stacks wouldn't fit back.",
                    "cart-deposit.collect");
        } else {
            // depositing
            ArrayList<ItemStack> transferitems = new ArrayList<>();
            ...
            for (ItemStack stack : transferitems)
                CraftBookPlugin.logDebugMessage(
                        "depositing " + stack.getAmount() + " items oftype " + stack.getType().toString(),
            ...
            CraftBookPlugin.logDebugMessage("deposited, " + transferitems.size() + " items left over.",
                    "cart-deposit.deposit");
            ...
            CraftBookPlugin.logDebugMessage("deposit done. " + transferitems.size() + " items wouldn't fit back.",
                    "cart-deposit.deposit");
        }
    }
