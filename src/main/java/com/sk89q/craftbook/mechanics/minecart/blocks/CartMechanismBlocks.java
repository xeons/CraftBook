...
/**
 * <p>
 * Stores the tuple of three blocks over which any typical CartMechanism is implemented, and also performs detection of
 * two of the blocks if only one is given.
 * </p>
 * <p>
 * Sign text and base block type are not validated by any constructors; this must be performed explicitly by calling the
 * appropriate methods after construction. Selection of signs thus does not concern itself with sign text at all; but
 * this is fine, since if you have two signs in an area where they could concievably contend for control of the
 * mechanism, you would be doing something that ought be physically impossible anyway (though yes, it is possible if
 * editing the world directly without physics).
 * </p>
 *
 * @author hash
 */
public class CartMechanismBlocks {
     ...
     * @param sign the block containing the sign that gives additional configuration to the mechanism, or null if not
     *        interested.
     */
     ...
     * unknown block appears to be a sign, rail, or base.
     *
    ...
    /**
     * <p>
     * Detecting factory, based on the position of the rails. The base must be one block below and the sign if it exists
     * must be two or three blocks below. Signs are guaranteed to be signs (unless they're null) and rails are
     * guaranteed to be rails.
     * </p>
     * <p>
    ...
    public static CartMechanismBlocks findByRail(Block rail) throws InvalidMechanismException {
        ...
        if (SignUtil.isSign(rail.getRelative(face, 2)))
            ...
            return new CartMechanismBlocks(rail, rail.getRelative(face, 1),
                    rail.getRelative(face, 1).getRelative(BlockFace.EAST, 1));
        else if (SignUtil.isSign(rail.getRelative(face, 1).getRelative(BlockFace.WEST, 1)))
            return new CartMechanismBlocks(rail, rail.getRelative(face, 1),
                    rail.getRelative(face, 1).getRelative(BlockFace.WEST, 1));
        else if (SignUtil.isSign(rail.getRelative(face, 1).getRelative(BlockFace.NORTH, 1)))
            return new CartMechanismBlocks(rail, rail.getRelative(face, 1),
                    rail.getRelative(face, 1).getRelative(BlockFace.NORTH, 1));
        else if (SignUtil.isSign(rail.getRelative(face, 1).getRelative(BlockFace.SOUTH, 1)))
            return new CartMechanismBlocks(rail, rail.getRelative(face, 1),
                    rail.getRelative(face, 1).getRelative(BlockFace.SOUTH, 1));
        return new CartMechanismBlocks(rail, rail.getRelative(face, 1), null);
    }

    /**
     * Detecting factory, based on the position of the base. The rails must be one block above and the sign if it exists
     * must be one or two blocks below. Signs are guaranteed to be signs (unless they're null) and rails are guaranteed
     * to be rails.
     *
     * @param base the block on which the rails sit; the type of this block is what determines the mechanism type.
     */
    private static CartMechanismBlocks findByBase(Block base) throws InvalidMechanismException {
        ...
        if (SignUtil.isSign(base.getRelative(BlockFace.DOWN, 1)))
            ...
            return new CartMechanismBlocks(base.getRelative(BlockFace.UP, 1), base,
                    base.getRelative(BlockFace.EAST, 1));
        else if (SignUtil.isSign(base.getRelative(BlockFace.WEST, 1)))
            return new CartMechanismBlocks(base.getRelative(BlockFace.UP, 1), base,
                    base.getRelative(BlockFace.WEST, 1));
        else if (SignUtil.isSign(base.getRelative(BlockFace.NORTH, 1)))
            return new CartMechanismBlocks(base.getRelative(BlockFace.UP, 1), base,
                    base.getRelative(BlockFace.NORTH, 1));
        else if (SignUtil.isSign(base.getRelative(BlockFace.SOUTH, 1)))
            return new CartMechanismBlocks(base.getRelative(BlockFace.UP, 1), base,
                    base.getRelative(BlockFace.SOUTH, 1));
        return new CartMechanismBlocks(base.getRelative(BlockFace.UP, 1), base, null);
    }

    /**
     * Detecting factory, based on the position of the sign. The base must be one or two blocks above and the rails an
     * additional block above the base. Signs are guaranteed to be signs and rails are guaranteed to be rails.
     *
    ...
    private static CartMechanismBlocks findBySign(Block sign) throws InvalidMechanismException {
        ...
        if (RailUtil.isTrack(sign.getRelative(BlockFace.UP, 2).getType()))
            ...
            return new CartMechanismBlocks(sign.getRelative(SignUtil.getBack(sign), 1).getRelative(BlockFace.UP, 1),
                    sign.getRelative(SignUtil.getBack(sign), 1), sign);
        throw new InvalidMechanismException("could not find rails.");
    }
    ...
    /**
     * This is a stupid but necessary thing since hash completely broke the ability to get the from location of the move
     * event from a mechanism itself.
     */
