...
public class SpeedModifiers extends AbstractCraftBookMechanic {
    ...
    public void onVehicleCreate(VehicleCreateEvent event) {
        ...
        if (unnoccupiedDeceleration > 0)
            ((Boat) event.getVehicle()).setUnoccupiedDeceleration(
                    ((Boat) event.getVehicle()).getUnoccupiedDeceleration() * unnoccupiedDeceleration);
        if (occupiedDeceleration > 0)
            ((Boat) event.getVehicle()).setOccupiedDeceleration(
                    ((Boat) event.getVehicle()).getOccupiedDeceleration() * occupiedDeceleration);
    }
    ...
    public void loadConfiguration(YAMLProcessor config, String path) {
        ...
        config.setComment(path + "unnoccupied-deceleration",
                "Sets the unnoccupied deceleration of a boat. -1 is disabled.");
        unnoccupiedDeceleration = config.getDouble(path + "unnoccupied-deceleration", -1);

        config.setComment(path + "occupied-deceleration",
                "Sets the occupied deceleration of a boat. 0.3 is normal occupied deceleration");
